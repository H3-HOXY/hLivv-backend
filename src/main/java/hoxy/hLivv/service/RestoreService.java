...
public class RestoreService {
    ...
    @Transactional
    public RestoreDto restoreRegister(RestoreRegisterDto restoreRegisterDto, MultipartFile imageFiles) {
        ...
        if (imageFiles != null) {
            try {
                var imageUrl = s3Service.uploadImage(S3Service.ImagePath.RESTORE, imageFiles);
                restoreRegisterDto.getRestoreImageUrls()
                                  .add(imageUrl);
            } catch (IOException e) {
    ...
    public List<RestoreDto> getRestores() {
        ...
        return restoreRepository.findByMember(member)
                                .stream()
                                .map(RestoreDto::from)
                                .toList();
    }
    ...
    public Page<RestoreDto> getAllProductsWithPagination(Pageable pageable) {
        return restoreRepository.findAll(pageable)
                                .map(RestoreDto::from);
//        return restoreRepository.findAll(pageable).stream().map(RestoreDto::from).toList();
    ...
    public RestoreDto getRestore(Long restoreId) {
        Restore restore = restoreRepository.findById(restoreId)
                                           .orElseThrow(() -> new NotFoundRestoreException("Re-store not found"));
        Member member = SecurityUtil.getCurrentUsername()
                                    .flatMap(memberRepository::findOneWithAuthoritiesByLoginId)
                                    .orElseThrow(() -> new NotFoundMemberException("Member not found"));
    ...
    public RestoreDto updateRestore(RestoreDto restoreDto) {
        Restore restore = restoreRepository.findById(restoreDto.getRestoreId())
                                           .orElseThrow(() -> new NotFoundRestoreException("Re-store not found"));
    ...
    public Integer getRestoreByRestoreStatus(RestoreStatus restoreStatus) {
        return restoreRepository.findByRestoreStatus(restoreStatus)
                                .size();
    }
